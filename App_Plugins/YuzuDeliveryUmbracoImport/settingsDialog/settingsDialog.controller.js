...
function settingsDialog($scope, formHelper, yuzuImportResources) {
    ...
    if (vm.dialogData.storeContentAs)
        ...
        vm.storeContentAs = {};
        vm.storeContentAs["$type"] = "YuzuDelivery.Umbraco.Import.InlineStoreContentAs, YuzuDelivery.Import";
    ...
    vm.change = function () {
        ...
        if (vm.storeContentAs.type === 0)
            newType["$type"] = "YuzuDelivery.Umbraco.Import.InlineStoreContentAs, YuzuDelivery.Import";
        else if (vm.storeContentAs.type === 1)
            newType["$type"] = "YuzuDelivery.Umbraco.Import.GlobalStoreContentAs, YuzuDelivery.Import";
        else if (vm.storeContentAs.type === 2)
            newType["$type"] = "YuzuDelivery.Umbraco.Import.GroupStoreContentAs, YuzuDelivery.Import";
